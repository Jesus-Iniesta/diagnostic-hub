...
import classes from './AcreditadorSidebar.module.css';

const NAV_ITEMS = [
  { id: 'inicio', label: 'Inicio', icon: IconHome, to: '/acreditador' },
] as const;

export type AcreditadorSidebarItemId = (typeof NAV_ITEMS)[number]['id'];

interface AcreditadorSidebarProps {
  active: AcreditadorSidebarItemId;
  collapsed?: boolean;
  onNavigate?: () => void;
}
...
export default function AcreditadorSidebar({
  active,
  collapsed = false,
  onNavigate,
}: AcreditadorSidebarProps) {
  const { user, logout } = useAuth();
  const navigate = useNavigate();

  const nombre = user?.nombre ?? 'Acreditador';
      ...
      <div>
        <Group className={classes.brand} gap="sm" wrap="nowrap">
          ...
          <div className={classes.brandText}>
            ...
            <Text className={classes.brandSubtitle}>Acreditador</Text>
          </div>
        </Group>
